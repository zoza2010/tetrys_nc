...
GEN_HDR_SIZE = 16
FLAG_META_XFER = 0x08  # META trailer: xfer + gen params
...
class MetaPacket:
    ...
    payload_size: int
    sha256_hex: str
    xfer: int = XFER_TETRYS  # XFER_TETRYS | XFER_GEN
    ...
    def pack(self) -> bytes:
        ...
        flags = FLAG_META_XFER if self.xfer == XFER_GEN else 0
        body = (
            _HDR.pack(MAGIC, VERSION, PKT_META, flags)
            + struct.pack("!QIH B", self.file_size, self.payload_size, len(name), len(digest))
            + name
            + digest
        )
        if flags & FLAG_META_XFER:
            body += struct.pack(
                "!BHHB",
                self.xfer & 0xFF,
    ...
    def unpack(cls, data: bytes) -> MetaPacket:
        ...
        if flags & FLAG_META_XFER and len(data) >= off + 6:
            xfer, gen_symbol_size, gen_k, gen_overhead_pct = struct.unpack_from(
                "!BHHB", data, off
            )
        return cls(
            file_size,
